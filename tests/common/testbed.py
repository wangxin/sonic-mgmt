...
class TestbedInfo(object):
    ...
    def calculate_dut_ptf_intf_index_map(self, tb):
        ...
        if 'host_interfaces' in topology:
            for _ports in topology['host_interfaces']:
                ...
                for port in str(_ports).split(','):
                    ...
                    map[dut_index][port_index] = int(ptf_index)
    ...
    def calculate_dut_ptf_intf_index_map_disabled(self, tb):
        map = defaultdict(dict)
        topo_facts = tb['topo']['properties']
        if 'topology' not in topo_facts:
            return map
    ...
    def calculate_ptf_dut_intf_index_map(self, tb):
        map = defaultdict(dict)
        for dut_index, dut_ptf_map in tb['topo']['ptf_map'].items():
            for dut_intf, ptf_intf in dut_ptf_map.items():
                map[str(ptf_intf)][dut_index] = int(dut_intf)
        return map
    ...
    def parse_topo(self):
        for tb_name, tb in self.testbed_topo.items():
            ...
            tb['topo']['ptf_map'] = self.calculate_dut_ptf_intf_index_map(tb)
            tb['topo']['ptf_map_disabled'] = self.calculate_dut_ptf_intf_index_map_disabled(tb)
            tb['topo']['ptf_dut_intf_index_map'] = self.calculate_ptf_dut_intf_index_map(tb)
