...
class FibTest(BaseTest):
    ...
    def setUp(self):
        ...
        if not self.src_ports:
            self.src_ports = [int(port) for port in self.ptf_port_dut_map.keys()]
    ...
    def check_ip_range(self, ip_range, dut_index, ipv4=True):
        ...
        for dst_ip in dst_ips:
            src_port, exp_ports, _ = self.get_src_and_exp_ports(dut_index, dst_ip)
            if not exp_ports:
            ...
            self.check_ip_route(src_port, dst_ip, exp_ports, dut_index, ipv4)
        ...
        if self.test_balancing and self.pkt_action == self.ACTION_FWD:
            for ip_range in ip_ranges:
                ...
                for i in range(0, self.balancing_test_times*len(exp_port_list)):
                    (matched_index, received) = self.check_ip_route(src_port, dst_ip, exp_port_list, dut_index, ipv4)
    ...
    def check_ip_route(self, src_port, dst_ip_addr, dst_port_list, dut_index, ipv4=True):
        if ipv4:
            res = self.check_ipv4_route(src_port, dst_ip_addr, dst_port_list, dut_index)
        else:
            res = self.check_ipv6_route(src_port, dst_ip_addr, dst_port_list, dut_index)
    ...
    def check_ipv4_route(self, src_port, dst_ip_addr, dst_port_list, dut_index):
        '''
    ...
    def check_ipv6_route(self, src_port, dst_ip_addr, dst_port_list, dut_index):
        '''
